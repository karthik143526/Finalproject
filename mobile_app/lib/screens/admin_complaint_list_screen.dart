...
class _AdminComplaintListScreenState extends State<AdminComplaintListScreen> {
  ...
  Future<void> _handleDelete(int id) async {
    final response = await ApiService.performAdminAction('delete_complaint', id);
    if (!mounted) return;

    if (response['status'] == 'success') {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(response['message'] ?? 'Complaint deleted'), backgroundColor: Colors.green),
      );
      _loadComplaints();
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
                              ...
                              rows: _complaints.map((row) {
                                ...
                                return DataRow(
                                  cells: [
                                    ...
                                    DataCell(Center(child: Text(row['type'].toString()))),
                                    DataCell(Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 250), child: Text(row['message'].toString(), overflow: TextOverflow.ellipsis, maxLines: 2)))),
